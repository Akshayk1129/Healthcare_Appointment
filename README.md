...
A comprehensive, concurrency-safe healthcare booking system built with React, Node.js, Express, and PostgreSQL (Prisma). It features atomic slot holds to prevent double-booking, robust background jobs for email notifications, AI-powered symptom analysis, seamless Google Calendar synchronization, an automated waitlist, and dynamic Medical Records PDF generation.
...
- `GET /api/admin/analytics` (Admin) - Fetch platform-wide analytics and AI triage data for the dashboard.
...
- `POST /api/appointments/waitlist` (Patient) - Join the automated waitlist for a specific date.
- `GET /api/appointments/export/pdf` (Patient) - Dynamically stream a Medical Records PDF generated by `pdfkit`.
...
- **WaitlistEntry:** Manages the automated queue system. Tracks `status` (`PENDING`, `NOTIFIED`, `CLAIMED`, `EXPIRED`) and automatically grants exclusive holds when slots free up.
